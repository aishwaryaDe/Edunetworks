...
namespace Millionlights.Controllers
{
    public class CourseController : Controller
    {
        ...
        public ActionResult EditCourse(int? id)
        {
            ...
            var courseContent = db.CourseContent.Where(x => x.Id == id).FirstOrDefault();

            CoursesDetails courseDetails = new CoursesDetails();
            if (id != null)
            {
                courseDetails.CourseId = course.Id;
                courseDetails.BasePrice = course.BasePrice;
                courseDetails.CertificationId = course.CertificationId;
                courseDetails.CertificationProvider = course.CertificationProvider;
                courseDetails.CourseCode = course.CourseCode;
                courseDetails.CourseAvailability = course.CourseAvailability;
                courseDetails.CourseCategory = course.CourseCategory;
                courseDetails.CourseDelivery = course.CourseDelivery;
                courseDetails.CourseImageLink = course.CourseImageLink;
                courseDetails.CourseLanguage = course.CourseLanguage;
                courseDetails.CourseLevels = course.CourseLevels;
                courseDetails.CourseName = course.CourseName;
                courseDetails.CourseProvider = course.CourseProvider;
                courseDetails.CourseTypes = course.CourseTypes;
                courseDetails.CreditPoints = course.CreditPoints;
                courseDetails.DeliveryID = course.DeliveryID;
                courseDetails.DisplayOnHomePage = course.DisplayOnHomePage;
                courseDetails.Duration = course.Duration;
                courseDetails.EDXCourseLink = course.EDXCourseLink;
                courseDetails.EnableForCertification = course.EnableForCertification;
                courseDetails.EnableForLMS = course.EnableForLMS;
                courseDetails.EnableForPayment = course.EnableForPayment;
                courseDetails.EndDate = course.EndDate;
                courseDetails.ExamManager = course.ExamManager;
                courseDetails.ExamObjective = course.ExamObjective;
                courseDetails.HasSampleContents = course.HasSampleContents;
                courseDetails.HasVideoLink = course.HasVideoLink;
                courseDetails.Hours = course.Hours;
                courseDetails.Instructor = course.Instructor;
                courseDetails.LMSCourseId = course.LMSCourseId;
                courseDetails.LongDescription = course.LongDescription;
                courseDetails.NoOfSessions = course.NoOfSessions;
                courseDetails.Objective = course.Objective;
                courseDetails.Rating = course.Rating;
                courseDetails.SampleContentsLink = course.SampleContentsLink;
                courseDetails.ScheduleApplicable = course.ScheduleApplicable;
                courseDetails.ShortDescription = course.ShortDescription;
                courseDetails.StartDate = course.StartDate;
                courseDetails.VideoLink = course.VideoLink;
                courseDetails.ChapterNumber = courseContent.ChapterNumber;
                courseDetails.ChapterName = courseContent.ChapterName;
                courseDetails.ChapterDescription = courseContent.ChapterDescription;
                courseDetails.EvidenceRequired = course.EvidenceRequired;
                courseDetails.CoursePriority = course.CoursePriority;

                courseDetails.UseDefaultCertificateContents = course.UseDefaultCertificateContents;
               
                courseDetails.CertificateTemplate = course.CertificateTemplate;
                courseDetails.CertificateLogo = course.CertificateLogo;
                courseDetails.CertificateSignature = course.CertificateSignature;
                courseDetails.CertificateTemplateHtmFile = course.CertificateTemplateHtmFile;  
            }
            List<CourseContent> coursecontent = db.CourseContent.Where(m => m.CourseId == id).ToList();
        ...
        public ActionResult GetCoursesByCategoryId(string clickedId, string type, string MinPrice, string MaxPrice)
        {
            ...
            string poratlEmail = System.Configuration.ConfigurationManager.AppSettings["PortalEmail"].ToString();
            ...
            foreach (var item in courseList)
            {
                ...
                if (isCourseEnrolled.Count() > 0)
                {
                    ...
                    p.EmailCourseName = "Hello, I have enrolled for the online course " + p.CourseName.Trim() + " , from " + newPartnerName + ". This course might interest you too. Please have a look on" + poratlEmail;
                ...
                else
                {
                    ...
                    p.GoogleCourseDescription = System.Uri.EscapeDataString(p.ShortDescription);
                    p.EmailCourseName = "Hello, I came across the online course " + p.CourseName.Trim() + " , from " + newPartnerName + ". This course might interest you. Please have a look on " + poratlEmail;
